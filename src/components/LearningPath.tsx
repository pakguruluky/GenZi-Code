...
export const LearningPath: React.FC<LearningPathProps> = ({
  ...
  const filteredMaterials = ALL_MATERIALS.filter(m => {
    ...
    if (searchQuery.trim()) {
      ...
      const seqStr = `${m.sequence}`;
      const modulStr = `modul ${m.sequence}`;
      const hashStr = `#${m.sequence}`;
      return (
        m.title.toLowerCase().includes(q) ||
        m.description.toLowerCase().includes(q) ||
        m.category.toLowerCase().includes(q) ||
        (m.competency && m.competency.toLowerCase().includes(q)) ||
        (m.indicators && m.indicators.toLowerCase().includes(q)) ||
        seqStr === q ||
        modulStr.includes(q) ||
        hashStr.includes(q)
      );
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800 space-y-4">
          ...
          <div className="relative">
            <div className="relative flex items-center">
              ...
              <input
                type="text"
                placeholder="Cari topik materi, modul coding (misal: Scratch, Loop, Sensor Microbit, PictoBlox AI, Game, Python)..."
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                className="w-full pl-11 pr-24 sm:pr-28 py-3 text-xs sm:text-sm rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/80 dark:bg-slate-800/80 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-900 transition-all shadow-inner"
              />
              <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
                ...
                <span className="hidden sm:inline-block px-2 py-1 text-[11px] font-bold rounded-lg bg-indigo-50 dark:bg-indigo-950/70 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
                  {filteredMaterials.length} Modul
                </span>
              </div>
            </div>

            {/* Quick Topic Search Chips */}
            <div className="flex items-center gap-1.5 overflow-x-auto pt-2.5 pb-1 text-xs no-scrollbar">
                ...
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold whitespace-nowrap transition-all shrink-0 border ${
                    searchQuery.toLowerCase() === tag.toLowerCase()
                      ...
                      : 'bg-white dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-700'
                  }`}
                >
                  {tag}
                </button>
      ...
      {filteredMaterials.length === 0 && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 sm:p-12 text-center border border-slate-200 dark:border-slate-800 shadow-xs max-w-xl mx-auto my-8">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-md mx-auto">
            Tidak ditemukan modul coding untuk pencarian <strong>"{searchQuery}"</strong> pada kategori dan level yang sedang dipilih.
          </p>
          <div className="mt-5 flex justify-center gap-2.5">
            <button
              ...
              className="px-4 py-2 rounded-xl text-xs sm:text-sm font-bold bg-indigo-600 hover:bg-indigo-700 text-white transition-colors flex items-center gap-1.5 shadow-xs"
            >
